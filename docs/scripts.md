...
| `fm-session-lock-lib.sh` | Shared session-lock ownership from harness ancestry or a trusted Claude session id, and the Claude Code registry proof that lets a conversation moved to the background take over its front-end's lock, for fm-lock.sh and the Claude Stop auto-arm, plus the read-only lock inspection behind `fm-lock.sh status`, `fm-inbox.sh ready`, and the process-event owner guard |
