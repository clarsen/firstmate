...
A live Claude front-end whose conversation Claude Code moved into this background session is not another session: `fm_session_lock_moved_to_self` in the same library owns that registry proof, and the guard then cooperates with the Stop auto-arm, which takes the lock over through `bin/fm-lock.sh`.
A genuinely other session cannot arm or repair the home without stealing the live owner's lock, so blocking it would create an unbounded loop; the lock-owning session remains responsible for restoring supervision.
...
- Claude registers two `Stop` hooks in `.claude/settings.json`, both anchored through `CLAUDE_PROJECT_DIR`: `bin/fm-turnend-guard.sh --claude`, and `bin/fm-claude-stop-autoarm.sh` with `asyncRewake: true` and `timeout: 28800`, twice the hook's renewal budget ([`watcher-continuity.md`](watcher-continuity.md#ownership) owns the renewal).
...
- Grok registers a `Stop` hook in `.grok/hooks/fm-primary-turnend-guard.json` and delegates capability selection to `bin/fm-turnend-guard-grok.sh`.
  ...
  A guard keyed on `GROK_AGENT` alone therefore stopped firing on grok 1.0.0, and the resulting Claude-only auto-arm ran synchronously under Grok - Grok has no `asyncRewake`, so it waited on the foregrounded watcher for the declared multi-hour timeout and the Grok turn never ended.
...
The Claude mode waits up to `FM_CLAUDE_AUTOARM_SYNC_WAIT_MS` (default 800 milliseconds) and allows the stop when the watcher is healthy, the auto-arm's generation claim is open, or `state/.claude-autoarm-epoch` contains a fresh exit-2 outcome, a rewake or a renewal, owned by this event epoch.
...
The hook catches HUP, TERM, and INT and commits the ordinary durable failed outcome and failure-notice marker before exiting 2 for a recovery turn; Claude settles its own timeout and teardown kills before signalling, so that exit 2 is never delivered for them, and the renewal bound keeps a quiet cycle inside the timeout instead.
