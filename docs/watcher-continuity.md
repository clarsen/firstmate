...
A numeric session-lock owner that fails the shared `fm_harness_pid_alive` predicate, or a live Claude front-end whose conversation Claude Code moved into this background session as `fm_session_lock_moved_to_self` proves, is taken over through `bin/fm-lock.sh` before auto-arm state changes, while any other live owner the session does not own, an absent lock, or a malformed lock keeps the competing hook inert.
...
The stale-owner or moved-conversation claim occurs only after the existing AFK and supervision-need gates pass.
...
Claude never delivers that exit 2 from a hook it timed out: it settles the hook as killed before signalling the hook's tree, so a quiet cycle left running into the registered timeout ends with no wake at all.
The hook therefore bounds every cycle it arms at `FM_CLAUDE_AUTOARM_RENEW_AFTER` seconds, half the registered timeout, so the renewal comes first under either clock Claude may time the hook with and across any system sleep shorter than that budget.
At the bound the watcher closes a quiet cycle at its terminal wait with one `renew:` line and leaves the recovery episode exactly as it was, so the next cycle opens without a catch-up wake.
The hook then commits the `renew` outcome and exits 2 with a renewal banner that asks for no handling, and the Stop ending that silent turn arms the next cycle.
An attached arm hands back at the bound and leaves the watcher it follows running for re-attach, and an owned watcher still running one guard grace past the bound is stopped as an interruption, which publishes downtime for the next start to re-present.
`bin/fm-claude-stop-autoarm.sh` and `bin/fm-watch-arm.sh` own the mechanics.
...
Every watcher close except a lifetime-bound renewal, and every durable queue append, publishes downtime, so a downtime republication of any pending episode reuses its generation instead of minting a new one, and an already-announced generation stays announced.
...
An actionable child output returns that reason normally, and a lifetime-bound `renew:` close returns that typed line the same way.
...
`tests/fm-watch-arm.test.sh` covers durable queue replay, real remote parent-replies ingestion into the authoritative status log, decision-only OPEN DECISIONS recovery, interrupted handling replay, generation-bound acknowledgement, a persistent live successor after recovery, a watcher close inside the handling window that must leave the printed acknowledgement valid, a re-arm whose recovery cycle is slowed after confirmation and must still surface rather than read as a watcher that stayed live, the self-healing moved-generation acknowledgement that consumes its handled rows and names its remedy, attached renewal that leaves the followed watcher running, the overrun backstop, and refusal of a malformed renewal bound.
...
`tests/fm-claude-stop-autoarm.test.sh` covers the auto-arm's scope, stale and live session owners, unchanged AFK and need boundaries, single-flight, bounded failure retries, benign live-watcher cycle ends, one-notice failure episodes, exit-2 translation, HUP/TERM/INT translation into the same durable failure handoff, the tracked registration's renewal margin inside its timeout, and a real quiet cycle renewing inside a simulated host timeout without a catch-up wake.
...
`FM_CLAUDE_RENEWAL_LIVE_E2E=1 tests/fm-claude-hook-renewal-live-e2e.test.sh` re-checks that Claude drops a timed-out hook's exit 2, then runs the tracked chain under a shortened timeout and requires silent renewals with no catch-up wake and no host kill.
...
Claude depends on the Stop `asyncRewake` rewake and on renewing each quiet cycle inside the registered hook timeout, which costs one silent turn per renewal budget of unbroken quiet, Cursor depends on its awaited stop-hook park, Grok retains native background-completion notifications, and Codex retains bounded foreground checkpoints.
