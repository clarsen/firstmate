...
The script runs while the spawn holds the project's slot-allocation lock, and a concurrent spawn of the same project refuses rather than waits, so keep it as short as the setup allows.
