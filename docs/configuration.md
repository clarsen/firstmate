...
A secondmate home's workers take their pooled copies from that home's own Treehouse root under `~/.treehouse/home-pools/`, never from the main home's pools, so inspect them with `treehouse --root <that root> status` from the home's clone; retirement and seed rollback remove that root with Treehouse's safe destroy, never forcing a copy there that is still leased, dirty, or unlanded (retirement then refuses, rollback warns); `fm_treehouse_home_root` in [`bin/fm-wake-lib.sh`](../bin/fm-wake-lib.sh) owns the exact path, and [Worktrees, not branches in your checkout](architecture.md#worktrees-not-branches-in-your-checkout) explains why.
...
The optional local, gitignored `config/project-setup/<project>.sh` prepares every new task worktree of one project before its worker starts, for setup a plain checkout lacks, such as cloning and installing a companion tool the project's agents need.
`<project>` is the name of the spawning project's directory, so `projects/taskbase` reads `config/project-setup/taskbase.sh`; a project with no script spawns exactly as before.
`bin/fm-spawn.sh` runs the script on every fresh ship and scout spawn, after the task worktree is proven isolated, clean, and current, and before any worker hook is written or the worker is launched.
It runs with the task worktree as its working directory, no standard input, and `FM_TASK_ID`, `FM_TASK_KIND`, `FM_PROJECT`, `FM_PROJECT_DIR`, and `FM_WORKTREE` in its environment, and its output is shown on the spawn's error stream.
`FM_PROJECT_SETUP_TIMEOUT` bounds it in seconds, 600 by default.
The script must be an executable regular file, and a script that is not, or a timeout that is not a positive whole number, refuses the spawn before any worktree or terminal is touched.
...
A home whose `state/.lock` names a live verified agent session, as `fm_session_lock_inspect` in [`bin/fm-session-lock-lib.sh`](../bin/fm-session-lock-lib.sh) decides it, is present even while nothing refreshes its lease, because an idle session whose supervision cycle has lapsed is not a gone home: stopping its listeners would leave a board answer sent later queued in Lavish with nothing polling it.
A free, stale, malformed, or unverifiable lock proves nothing, so the lease alone decides for a home no live session holds.
KNOWN LIMIT: while any activity continues, or any live agent session holds the home, in a home whose original owning session has ended, a runner of that home keeps running until its source is retired or the home goes away.
...
That guard accepts the lease only while the state root retains the device/inode identity recorded by the runner's claim, and initiates the verified stop after two consecutive reads cannot prove that identity together with either lease freshness or a live agent session holding the home, so one unreadable read cannot kill a live runner.
...
FM_PROCEVENT_OWNER_LEASE_SECONDS=600    # how long a source runner keeps going with no activity and no live session in its owning home; 1..86400
...
FM_CLAUDE_AUTOARM_RENEW_AFTER=14400   # seconds a quiet Stop-owned Claude cycle runs before one silent renewal turn; keep at most half the registered auto-arm hook timeout (docs/watcher-continuity.md)
