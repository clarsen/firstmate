...
A task with edit members lands one repository at a time, so after each merge steer its worker to the next repository in land order rather than cleaning up ([`docs/configuration.md`](docs/configuration.md) "Task members").
